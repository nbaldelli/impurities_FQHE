# -*- coding: utf-8 -*-
"""
Created on Thu Apr  9 15:23:58 2020
...
@njit
def listocc(allchoices,lmax):
    occ=[0]*(lmax+1)
    for i in range(len(allchoices)):
        occ[lmax-allchoices[i]]+=1
    return occ

@njit
def occlist(hbin,lmax):
    states=[]
    for i in range(len(hbin)):
        if hbin[lmax-i]!=0:
            states.extend([i]*hbin[lmax-i])
    return states
...
if (Nh>0):  
    ...
    for i in range(len(allchoices)):
        ...
        if (sector>=0)and(sector<d):
            ...
            states.append(allchoices[i,:])
            sectors.append(sector)
            hilbert.append(Dh[sector])

    hbase=np.zeros((len(hilbert),len(sectors),Nh),dtype=np.int64)
    hbase[np.array(hilbert)-1,sectors]=states
    #del(states,hilbert,sectors)
...
for sec in range(d): # loop over all sectors
    ...
    for ih in range(Dh[sec]): # loop over all impurity configuration in the sector
        ...
        if (Nh>0): # if we have no impurities, the impurity has not been defined, therefore the "if" 
            hbin=listocc(list(hbase[ih,sec,:]),lmax)
            hoccs=[0,0]
            hoccs[:]=np.unique(hbase[ih,sec,:])
                        ...
                        for l4 in range(lmax+1):
                           ...
                           if (l3>-1)and(l3<lmax+1)and(newebin[lmax-l4]==0):    
                               ...
                               if ((np.dot(lmax-larr,newebin))<=Lmax):
                                   aa=len(np.where(eoccs<l1)[0]) # how many occupied levels must electron annihilator pass
                                   eoccs2=np.setdiff1d(eoccs,l1)
                                   bb=len(np.where(eoccs2<l4)[0])
                                   sig=(-1)**(aa+bb) #devo capire che segno prendono i fermionic operators
                                   v2=np.sqrt(hbin[lmax-l2])*np.sqrt(newhbin[lmax-l3])*sig*vmin[l4,l3,l2,l1] # vhal(l4,l3,l2,l1,1,0);
#                                   print('pot')
#                                   print(v2)
                                   ieout=int(elist[int(np.dot(newebin,pots)),1])
                                   ihout=int(hlistf(hbase,np.array(occlist(newhbin,lmax))))
...
np.savetxt('vals'+str(L)+str(Ne)+str(Nh),vals)
np.savetxt('angm'+str(L)+str(Ne)+str(Nh),L-Lel)
np.savetxt('stdev'+str(L)+str(Ne)+str(Nh),std)
print(vals)
print(L-Lel)
print(std)
